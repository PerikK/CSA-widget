...
const BASE_URL = (process.env.UPSTREAM_URL ?? "https://api.enora-oah.eu").replace(
  /\/+$/,
  ""
);
...
async function login(): Promise<string> {
  ...
  if (!res.ok) {
    throw new Error(`Upstream login failed with status ${res.status}`);
  }
...
async function getToken(force = false): Promise<string> {
  if (!force && cachedToken && cachedToken.expiresAt > Date.now() + EXPIRY_SKEW_MS) {
...
export async function getSubmissions(): Promise<CitizenSubmission[]> {
  ...
  if (!res.ok) {
    throw new Error(`Upstream submissions request failed with status ${res.status}`);
  }
